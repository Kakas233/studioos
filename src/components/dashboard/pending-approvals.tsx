    ...
    <Card className="bg-amber-500/[0.06] border-amber-500/10">
      <CardHeader className="pb-3">
        <CardTitle className="text-lg font-medium text-amber-400 flex items-center gap-2">
          ...
          Pending Approvals
          <Badge className="bg-amber-500/20 text-amber-400 border-0 ml-2">
      ...
      <CardContent className="space-y-4">
        {pendingRequests.map((request) => {
          ...
          return (
            <div
              key={request.id}
              className="bg-[#111111]/60 rounded-lg border border-white/[0.04] p-4 space-y-3"
            >
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <p className="font-medium text-white">
                    {formatUsd(oldData?.total_gross_usd || 0)}
                  </p>
                  <p className="text-[#A8A49A]/30 text-xs">
                    {formatSecondary(oldData?.total_gross_secondary || 0)}
                  </p>
                </div>
                <div>
                  <p className="text-[#A8A49A]/40 text-xs mb-1">New Value</p>
                  <p className="font-medium text-emerald-400">
                    {formatUsd(newData?.total_gross_usd || 0)}
                  </p>
                  <p className="text-emerald-400/60 text-xs">
                    {formatSecondary(newData?.total_gross_secondary || 0)}
                  </p>
                </div>
              </div>
              <div className="flex gap-2 pt-2">
                ...
                <Button
                  ...
                  className="flex-1 text-red-400 hover:text-red-300 border-red-500/20 hover:bg-red-500/10"
                >
