      ...
      {payload.map((p, i) => (
        <div key={i} className="flex items-center gap-2 py-0.5">
          ...
          <span className="text-[11px] text-[#A8A49A]/50">
            {p.dataKey === "gross_usd" ? "Revenue" : p.dataKey}:
          </span>
...
export default function RevenueChart({
  ...
  const fmtSec = formatSecondary(totalSecondary);
    ...
    <div className="bg-[#111111] border border-white/[0.04] rounded-xl p-5">
      <div className="flex items-start justify-between mb-5">
        <div>
          ...
          <p className="text-xs text-[#A8A49A]/30 mt-0.5">{fmtSec}</p>
        </div>
          ...
          <button
            ...
            className={`p-1.5 rounded-md transition-colors ${
              chartType === "area"
                ...
                : "text-[#A8A49A]/30 hover:text-[#e8e6e3]"
            }`}
          >
          ...
          <ResponsiveContainer width="100%" height="100%">
            {chartType === "area" ? (
              ...
              >
                ...
                <XAxis
                  dataKey={
                    rangeData.length > 14 ? "shortDate" : "displayDate"
                  }
                  tick={{
                    fontSize: 10,
                    fill: "rgba(168,164,154,0.35)",
                  }}
                  tickLine={false}
                  axisLine={false}
              ...
              >
                ...
                />
                <Tooltip content={<CustomTooltip />} />
                <Bar dataKey="gross_usd" radius={[4, 4, 0, 0]}>
                  {rangeData.map((d, i) => {
                    const intensity =
                      maxVal > 0 ? d.gross_usd / maxVal : 0;
                    return (
                      <Cell
                        key={i}
                        fill="#C9A84C"
