...
// 10. Support Ticket Escalation (internal)
export async function sendEscalationEmail(
  ...
  category: string,
  agent: string,
  messages: Array<{ role: string; content: string; timestamp?: string; sender_name?: string }>
): Promise<SendResult> {
  const shortId = ticketId.substring(0, 6).toUpperCase();

  const messagesHtml = messages
    .map((msg) => {
      const bgColor = msg.role === "user" ? "rgba(201, 168, 76, 0.08)" : "rgba(255,255,255,0.03)";
      const label = msg.role === "user" ? (msg.sender_name || customerName) : `AI Agent (${agent})`;
      const labelColor = msg.role === "user" ? "#C9A84C" : "#A8A49A";
      return `
        <div style="background: ${bgColor}; border-radius: 8px; padding: 12px; margin-bottom: 8px;">
          <p style="color: ${labelColor}; font-size: 11px; font-weight: 600; margin: 0 0 4px 0;">${label}</p>
          <p style="color: #ffffff; font-size: 13px; line-height: 1.6; margin: 0;">${msg.content}</p>
          ${msg.timestamp ? `<p style="color: rgba(168,164,154,0.3); font-size: 10px; margin: 4px 0 0 0;">${msg.timestamp}</p>` : ""}
        </div>
      `;
    })
    .join("");
  ...
  const html = emailWrapper(`
    <div style="background: rgba(201, 168, 76, 0.1); border: 1px solid rgba(201, 168, 76, 0.3); border-radius: 12px; padding: 16px; margin-bottom: 24px;">
      <h2 style="color: #C9A84C; font-size: 16px; font-weight: 600; margin: 0;">Escalated Ticket #${shortId}</h2>
      <p style="color: #A8A49A; font-size: 13px; margin: 4px 0 0 0;">${subject}</p>
    </div>
    <table style="width: 100%; margin-bottom: 24px;">
      <tr><td style="color: rgba(168,164,154,0.5); font-size: 12px; padding: 4px 0;">Reason:</td><td style="color: #fff; font-size: 12px; padding: 4px 0;">${reason}</td></tr>
      <tr><td style="color: rgba(168,164,154,0.5); font-size: 12px; padding: 4px 0;">Customer:</td><td style="color: #fff; font-size: 12px; padding: 4px 0;">${customerName} (${customerEmail})</td></tr>
      <tr><td style="color: rgba(168,164,154,0.5); font-size: 12px; padding: 4px 0;">Studio:</td><td style="color: #C9A84C; font-size: 12px; padding: 4px 0;">${studioName}</td></tr>
      <tr><td style="color: rgba(168,164,154,0.5); font-size: 12px; padding: 4px 0;">Category:</td><td style="color: #fff; font-size: 12px; padding: 4px 0;">${category}</td></tr>
      <tr><td style="color: rgba(168,164,154,0.5); font-size: 12px; padding: 4px 0;">Agent:</td><td style="color: #fff; font-size: 12px; padding: 4px 0;">${agent}</td></tr>
    </table>
    <p style="color: rgba(168,164,154,0.5); font-size: 12px; text-transform: uppercase; letter-spacing: 0.05em; margin: 0 0 12px 0;">Conversation</p>
    ${messagesHtml}
  `);

  return sendEmail(
    "support@getstudioos.com",
    `[Escalated] Ticket #${shortId} \u2014 ${subject}`,
    html,
    "StudioOS Support <noreply@getstudioos.com>"
  );
}
